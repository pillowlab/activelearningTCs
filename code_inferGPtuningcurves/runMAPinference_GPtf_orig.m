function [fmap,fstd,phimap,phicov,gpstat] = runMAPinference_GPtf_orig(xx,dat,gfun,theta,phi0)
% Compute MAP estimate for tuning function under GP-Poisson model
% [fmap,fstd,phimap,phicov,gpstat] = runMAPinference_GPtf_orig(xx,dat,gfun,theta)
%
%  Model:
...
%
% Note: this version projects multiplies phi by matrix to evaluate at every
% r value. This approach may be useful for version with over-dispersed
% noise, where we don't have a sufficient statistic.  But for this version
% it's better to combine r's for each stimuli, and just keep track of how
% many times each stimulus was presented.
...
if rcond(K)< (1/condthresh);
    ...
    Bproj = Mid*Uproj; % projection from optimization params to GP inputs
    ...
    Bproj = Mid; % projection from optimization params to GP inputs
end

% Set up optimization of log-posterior
lossfun = @(prs)(neglogpost_GPtf(prs,Bproj,rtr,ones(ntr,1),gfun,mu,Kinv)); % loss function 
opts = optimset('display', 'off','gradobj','on','Hessian','on');
